...
public class Release_padlock extends DalBaseProcess{
	...
	public void padlockReleaseOperation() {
		...
		throw new OBException("No es una instancia de produccion");
		 ...
		 try {
			    ...
			    /*final OBCriteria<DocumentType> queryDocumenType= OBDal.getInstance().createCriteria(DocumentType.class);
			    queryDocumenType.add(Restrictions.eq(DocumentType.PROPERTY_DOCUMENTCATEGORY,"ARI"));
			    ...
			    String[] docTypeIds = new String[listDocType.size()]; // Creamos un array del tamaño adecuado
			    int index = 0;
				...
				String docopp="";
			    /*
				final OBCriteria<Invoice> queryInvoice= OBDal.getInstance().createCriteria(Invoice.class);
			    queryInvoice.add(Restrictions.in(Invoice.PROPERTY_TRANSACTIONDOCUMENT + ".id", docTypeIds));//ids
			    queryInvoice.add(Restrictions.eq(Invoice.PROPERTY_OUTSTANDINGAMOUNT, BigDecimal.ZERO));
			    queryInvoice.add(Restrictions.eq(Invoice.PROPERTY_SHPICOPERATIONSTATE,"05"));//CANCELADO
			    queryInvoice.add(Restrictions.eq(Invoice.PROPERTY_SHPICLOCKSTATUS,"001"));//001->DESBLOQUEADO //002->BLOQUEADO
			    ...
			    */
			    if(headerData.length > 0) {
			    	for(ReleasePadLockData data : headerData) {
				    	...
				    	queryOpportunity.add(Restrictions.eq(Opcrmopportunities.PROPERTY_SHPPWSOPDOCUMENTNO, objInvoice.getDocumentNo()));
	...
	public boolean getApiResponsePadlockRelease(shppws_config accesApi, String Identifier) throws Exception {
	    ...
	    if (responseCode == HttpURLConnection.HTTP_OK) {
	        ...
	        try {
				logger.log_end_register(log, apiUrl, noReference,apiResponse, "OK", "OUT", Interface, Process, idRegister, Error);
				return true;
	        	
	        }catch(Exception e) {
	        	logger.log_end_register(log, apiUrl, noReference, apiResponse, "ERROR", "OUT", Interface, Process, idRegister, Error);
		        return false;
	        }
		    
	    } else {
	    	logger.log_end_register(log, apiUrl, noReference, "Response Code "+responseCode, "ERROR", "OUT", Interface, Process, idRegister, Error);
	    }
	    return false;
	}
	
	public String getOAuth2Token(CloseableHttpClient httpClient, String tokenUrl, String clientId, String clientSecret, String scope) {
		try {
	        // New Post
